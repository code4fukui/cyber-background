...
export class RoundCameraController
{
  ...
  public enable():void
  {
    ...
    if ("ongesturestart" in window)
    {
      ...
      this._stage.addEventListener(MSGestureEventName.GESTURE_CHANGE, (event:MSGestureEvent) =>
      {
        this._gestureChangeHandler(event);
      });
      document.addEventListener(MSGestureEventName.GESTURE_END, (event:MSGestureEvent) =>
      {
  ...
  private _wheelHandler(event:MouseWheelEvent):void
  {
